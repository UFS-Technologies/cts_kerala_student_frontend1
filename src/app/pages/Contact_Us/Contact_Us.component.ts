...
Contact_Subject: string = 'Student Admission';
...
Tab_click(Current_tab)
{
  ...
  async Submit_Contact_Form_Click() {
    if (!this.Contact_Name || this.Contact_Name.trim() === '') {
      this.dialogBox.open(DialogBox_Component, { panelClass: 'Dialogbox-Class', data: { Message: 'Please enter your name', Type: "2" } });
      return;
    }
    if (!this.Contact_Email || this.Contact_Email.trim() === '') {
      this.dialogBox.open(DialogBox_Component, { panelClass: 'Dialogbox-Class', data: { Message: 'Please enter your email', Type: "2" } });
      return;
    }
    if (!this.Contact_Phone) {
      this.dialogBox.open(DialogBox_Component, { panelClass: 'Dialogbox-Class', data: { Message: 'Please enter your phone number', Type: "2" } });
      return;
    }
    if (!this.Contact_Message || this.Contact_Message.trim() === '') {
      this.dialogBox.open(DialogBox_Component, { panelClass: 'Dialogbox-Class', data: { Message: 'Please enter your message', Type: "2" } });
      return;
    }
    ...
    this.CandidateService.Submit_Contact_Form(payload).subscribe(
      (response: any) => {
        this.issLoading = false;
        if (response && response.status === 1) {
          this.dialogBox.open(DialogBox_Component, { panelClass: 'Dialogbox-Class', data: { Message: 'Your message has been sent successfully!', Type: "3" } });
          // Clear fields
          this.Contact_Name = '';
          this.Contact_Email = '';
          this.Contact_Phone = '';
          this.Contact_Subject = 'Student Admission';
          this.Contact_Message = '';
        } else {
          this.dialogBox.open(DialogBox_Component, { panelClass: 'Dialogbox-Class', data: { Message: response.message || 'Failed to send message', Type: "2" } });
        }
      },
      (error) => {
        this.issLoading = false;
        this.dialogBox.open(DialogBox_Component, { panelClass: 'Dialogbox-Class', data: { Message: 'Error sending message. Please try again.', Type: "2" } });
      }
    );
  }
}